...
const writeLogFile = (req, res, error) => {
    ...
    if (!fileExists) logStream.write('method | email | url | request body | status code | error | hour \n');
    ...
    const requestDetails = `${req.method} | ${req.metadata['email']} | ${decodeURI(req.originalUrl)} | ${JSON.stringify(reqBodyChangePass)} | ${error?.statusCode || res.statusCode} | ${error} | ${hour} \n`;
